...
<Tabs>
  <Tab title="In-browser (on this device)">
    ...
    **Best for:** a few dozen to a few hundred documents. No install, no server, no external dependencies. Works offline after the initial model download.
    ...
    You can [read the query-rewriting instruction](/developers/system-prompts-chat#prompt-lens-rewrite). It receives your question, not your lab values or profile details.
  </Tab>
...
See [what getbased asks AI](/guides/ai-prompts#when-you-send-a-chat-message) to understand where those passages appear in the full chat request.
