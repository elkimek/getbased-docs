...
Common unit differences are normalized automatically. For example, enzymes reported as IU/L are mapped safely to the marker's canonical unit for storage. Your selected International, Australia/NZ, or US profile then controls how the saved value and range appear. The marker detail modal can also show alternate units when **Alternate Units** is enabled in **Settings → Display**. See [Choose units for your region](/guides/unit-profiles).
