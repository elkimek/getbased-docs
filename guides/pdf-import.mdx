...
You can also start the same reviewed import workflow from **Chat → + → Import health file**, or by dropping a PDF over chat. The PDF is handled by getbased's importer, not attached as an arbitrary file to an agent session. When **CLI agents** is selected, the chosen local adapter/model must support the required text or image feature; personal gateways are chat-only. If native file dropping is denied by the browser, choose the file through the picker instead. See [CLI agents](/guides/cli-agents).
