...
| **WHOOP** | Experimental OAuth 2.0 | Self-host only. Requires the operator's WHOOP developer app, client ID, client secret, and explicit enable flag. Hidden on unconfigured hosted deployments. |
...
- **Raw daily rows stay device-local after import.** Google Health rows are always browser-device encrypted. Other provider rows use passphrase encryption when **Settings → Data → Security** protection is enabled; without it, they remain readable inside the browser profile.
...
| `WHOOP_ENABLED` | WHOOP | Must be exactly `true`; experimental self-host integration. |
...
On `localhost`, WHOOP and Ultrahuman appear as **experimental · setup required** until configured. On other unconfigured hosted domains, they are hidden. A fully configured self-host shows **experimental · self-hosted** and enables **Connect**. If an operator later disables either provider, existing connections remain removable but sync and reconnect pause.
