...
The xAI and ElevenLabs voice keys are stored encrypted in the browser and are included only in encrypted sync when you enable cross-device sync. A compatible-server key and server URL stay on the current device. See [Voice privacy and storage](/guides/privacy#voice-processing) for the full boundary.
