...
Light environment data belongs to the **Light & Sun** source. It appears in in-app AI chat and light-related score context when **Insight → Manage → Context → Data sources → Light & Sun context** is on. With that source enabled, the AI context can include:
...
A user who walks 15 minutes outside but sits under cool LEDs for 11 hours has a fundamentally different photobiological day than someone who stays indoors for 2 hours under warm light in the evening. The AI can see that picture when the Light & Sun source is enabled; turning it off does not delete your logged rooms, screens, or measurements.
