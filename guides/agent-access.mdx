---
title: "Connect getbased to external AI assistants"
sidebarTitle: "Agent Access"
description: "Connect MCP-compatible AI assistants to getbased with a private setup command. The gateway stores only encrypted context; decryption happens locally."
---
...
Looking to use an installed agent **inside getbased chat**? Use **Settings → AI → CLI agents** and [the Companion guide](/guides/cli-agents) instead. That path does not require relay Agent Access or Sync, and its health tools respect the in-app Data sources switches. The relay-sharing permission described on this page is separate.
...
Agent Access encrypts the same structured context text assembled for in-app AI, then exposes its named sections through the local MCP client. Depending on the profile, that can include:

- lab values, effective reference ranges, trends, specialty lab groups, marker notes, and flagged-result summaries;
...
Answered context fields remain meaningful even when they rule something out. The summary includes skin type and explicit digestive answers such as **None** or **Normal**; unanswered fields are omitted rather than guessed.

The in-app **Data sources** switches do not filter Agent Access. External sharing is controlled by the separate **Settings → Agent Access** toggle because the browser must produce a consistent complete summary for the MCP tools. The gateway receives only the encrypted envelope; plaintext becomes available only after your local MCP client decrypts it with `GETBASED_AGENT_CONTEXT_KEY`.
...
<Steps>
  ...
  <Step title="Install getbased-agent-stack (public agent path)">
    ...
    <CodeGroup>
      ...
      getbased-stack init --yes --local-only   # scaffolds config and starts services where supported
      ```
    </CodeGroup>
...
For time-series reasoning — "did my HRV drop the week before I got sick?" — the compact summary is not enough. Open **Settings → Agent Access** and turn on **Push wearable daily series**. getbased then writes a pivoted daily matrix into a separate section:
...
If you log Sun & Light sessions, the agent can pull them via `getbased_section('sun')`. The section is generated from the same Light & Sun context as the browser: sun sessions, light-device sessions, indoor light environment, channel doses, atmosphere, eye-exposure mode, and body-exposure summaries when present.
