...
description: "Connect any MCP-compatible AI agent to your getbased lab context using a relay token plus a separate local decryption key. Raw data and sync secrets stay in your browser."
---
...
<Steps>
  <Step title="Enable Sync and Agent Access">
    ...
    4. Copy both generated values:
       - **Read-only token** → `GETBASED_TOKEN`
       - **Context encryption key** → `GETBASED_AGENT_CONTEXT_KEY`

    <Note>
      The token and context key are masked by default. Click **Show** to reveal them. Treat both like passwords — do not commit them to a public repository or share them publicly.
    </Note>
  </Step>
  ...
  <Step title="Add both Agent Access values to your agent's config">
    Paste both `GETBASED_TOKEN` and `GETBASED_AGENT_CONTEXT_KEY` into your agent's MCP configuration. See the [Hermes Agent example](#hermes-agent-config-example) below, or open the dashboard at `http://127.0.0.1:8323` for auto-generated config for Claude Code, Claude Desktop, Cursor, Cline, Windsurf, and more.
  </Step>
...
Replace `your-token-here` and `your-context-key-here` with the two values from **Settings → Agent Access**. The token authorizes the relay fetch; the context key decrypts the payload locally in your MCP process.
