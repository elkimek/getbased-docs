...
description: "Record room-by-room EMF readings, rate them against building-biology thresholds, and include them in profile context and recommendations."
---

The EMF Assessment is for measured electromagnetic-field readings in the places where you spend time. It lives inside the **Environment** context card and is included in export/import, sync, and recommendations. It reaches in-app AI context when **Insight → Manage → Context → Data sources → Insight Context Cards** is enabled.
...
<Info>
  Read the complete [EMF interpretation instruction](/developers/system-prompts-insights#prompt-insight-emf). If you import a report, the separate [EMF extraction instruction](/developers/system-prompts-imports#prompt-import-emf) shows the fields the AI is asked to find.
</Info>
...
- **AI chat** receives a summary of saved EMF assessments as part of profile context when **Insight Context Cards** are enabled in Manage → Context.
