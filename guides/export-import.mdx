...
A database bundle downloads containing every profile's complete stored data, chat history, threads, custom personalities, and settings. This includes device-local AI model-test history that is not present in a curated single-profile export. Use the bundle for a complete backup or when migrating your entire getbased setup to a new browser or device.
