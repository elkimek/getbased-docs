...
## Unit-profile boundary

Marker results and ranges are stored once in the canonical units declared by the schema. `unit-profiles.js` projects all 196 built-in markers through three persisted display profiles: `EU` for International (SI), `ANZ` for common Australia/New Zealand pathology units, and `US` for US conventional units. The legacy `EU` identifier remains part of profile persistence for compatibility.

`getActiveData()` applies the projection after calculations and range resolution. Tables, charts, marker details, reports, manual entry, and AI context consume that same result. Manual entry and import review accept compatible regional and secondary units and convert them back to canonical storage. Custom markers retain their user-supplied canonical unit.

Do not convert again in an individual renderer. Add regional differences to the central profile or secondary-unit registries, record conversion provenance where needed, and round-trip the complete catalog in `unit-profiles.test.js` plus focused active-data and range-shape tests.
...
Compare Dates must resolve this contract independently for both selected dates. `compare-correlations.js` uses `resolveMarkerRangeContext()` to render every displayed range, marks the range used for status when more than one is visible, and groups the ranges by date when their signatures differ. Direction coloring compares normalized distance outside each date's applicable range; it must not score movement toward a static midpoint as improvement.
...
| Active data, calculations, and display units | `js/data.js`, `js/data-calculated-markers.js`, `js/marker-context-ranges.js`, `js/unit-profiles.js`, `js/secondary-unit-conversions.js` |
...
| Effective range and comparison UI | `js/marker-analysis.js`, `js/compare-correlations.js`, chart and marker-detail modules |
