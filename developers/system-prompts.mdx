...
<Warning>
  Application source is authoritative. Prompt text shown in this catalog is normalized for readability: runtime values are replaced with `{placeholders}` and shared fragments are named once. A prompt change is incomplete until the source, this catalog, and directly related tests agree.
</Warning>
...
- visible, editable suggestion buttons and prefilled questions;
...
Visible **Ask AI** templates for markers, correlations, recommendations, Genome findings, and Biology Score coverage inherit the normal chat system prompt. They are user messages rather than separate privileged instructions.
