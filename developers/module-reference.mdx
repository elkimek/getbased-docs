...
Private-transport attestation display helpers shared by the chat header and assistant-message cost footnotes. Tinfoil's fully verified state uses **🔒✓**; Venice binding-only evidence uses **🔒~** so the UI does not imply full DCAP, measurement, GPU, or response-signature verification.
