...
Encrypted single-profile share links. The module creates a profile export through the module-only `buildClientExportObject()` helper, encrypts it in the browser with a password, and imports shared profiles from `#share/{id}` deep links after local password decryption. New `vps1_` ids on official origins go directly to the isolated profile-share service; legacy ids retain the same-origin `/api/share` compatibility path during the bounded transition. Shared envelopes require at least 100,000 PBKDF2-SHA256 iterations; the normal generated value is 600,000.
...
- `voice-provider-xai.js` and `voice-provider-elevenlabs.js` use `voice-provider-cloud-shared.js` to call the bounded same-origin `/api/voice` relay.
...
**Server boundary:** `api/voice.js` delegates to `lib/voice-relay-handler.js`; `lib/voice-relay-policy.js` allowlists providers/actions, bounds request and response sizes, validates identifiers and languages, and maps normalized requests to xAI or ElevenLabs.
...
**OAuth2 (server-side secret):** `wearables-oura.js` + `…-oura-auth.js`, `…-withings.js` + `…-withings-auth.js`, `…-ultrahuman.js` + `…-ultrahuman-auth.js`, `…-polar.js` + `…-polar-auth.js`, and `…-google-health.js` + `…-google-health-auth.js`. Secrets live in `.env.local` + `/api/proxy`. Google Health additionally requires the explicit server capability gate and encrypts its credentials and raw rows with its device-local credential vault key.

**OAuth2 PKCE (no secret):** `wearables-whoop.js` + `…-whoop-auth.js`. The Fitbit PKCE modules remain only to sync already-connected legacy users during the migration window; the adapter is hidden for new connections.
