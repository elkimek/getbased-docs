...
Chat message action bar rendering and message-level handlers. The module owns `Regenerate`, `Copy`, `Listen`, and context-details toggle behavior while calling back to `window.renderChatMessages?.()`, `window.sendChatMessage?.()`, and `window.isChatStreaming?.()` to avoid importing the streaming/rendering module. Voice actions cross the lazy boundary through `voice-loader.js`.
...
- `sync-push.js` — outbound profile push path, unchanged-payload suppression, generation-safe dirty clearing, exact source-commit tracking, relay-storage refresh scheduling, in-flight watchdog, and Evolu profileData insert/update.
...
- `sync-actions.js` — user-triggered sync actions, all-profile initial push, dirty-profile compaction preflight, relay snapshot rebuild, and compatibility re-exports for save hooks and storage cleanup.
- `sync-save-hooks.js` — save/chat/profile-metadata debounce hooks, durable dirty marking, AI-setting push scheduling, per-profile timer cleanup, and active or stored profile reads for pushes.
