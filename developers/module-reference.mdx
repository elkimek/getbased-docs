...
description: "Reference map for the getbased native ES modules and compatibility exports."
---
...
Modules live under `js/`. The app has hundreds of focused top-level JavaScript modules. This page is a guided implementation map, not a promise that every small hook, delegate, or helper file has its own long section. It is grouped by layer; lower layers have no dependencies on higher ones.

---

## Layer 1 — Foundation

### `schema.js` and `marker-schema/`

`schema.js` is the stable compatibility facade for the modular biomarker catalog, range registries, unit conversions, pricing, and environment thresholds. Category definitions and immutable marker identities are authored in category modules under `marker-schema/`, composed by `marker-schema/index.js`, and generated into `marker-schema.js`; contributors must not edit the generated file directly.

**Key exports:**
- `MARKER_SCHEMA` — 196 core entries across 19 categories, including the 22-entry `calculatedRatios` category
- `UNIT_CONVERSIONS` — keyed by `"category.markerKey"`: `{ type: 'multiply', factor, unit }` for canonical→US compatibility; schema-wide profile resolution lives in `unit-profiles.js`
...
- `CONTEXT_REFERENCE_RANGES` / `CONTEXT_OPTIMAL_RANGES` — age-, sex-, and date-resolved guidance consumed by the active-data pipeline
...
- `BUILTIN_MARKER_IDENTITIES`, `BUILTIN_MARKER_DOT_KEY_ALIASES`, `getBuiltinMarkerId()`, `getBuiltinMarkerDotKey()`, and `resolveBuiltinMarkerDotKey()` — immutable `gb:marker:*` IDs plus current and legacy location compatibility exports
...
### `marker-schema/index.js` and `marker-schema.js`

`marker-schema/index.js` is the authoring composition root. It imports the category source modules in stable order and exports the catalog inputs. `scripts/build-marker-schema.mjs` turns those inputs into the generated runtime module `marker-schema.js`; contributors must not edit the generated file directly.

**Key generated exports:** `MARKER_SCHEMA`, `BUILTIN_MARKER_IDENTITIES`, `BUILTIN_MARKER_DOT_KEY_ALIASES`, `getBuiltinMarkerId()`, `getBuiltinMarkerDotKey()`, `resolveBuiltinMarkerDotKey()`, `CUSTOM_MARKER_ID_PREFIX`, and `isCustomMarkerId()`.

See [Marker model and category placement](/developers/marker-model) for the authoring and migration contract.

---

### `unit-profiles.js` and `secondary-unit-conversions.js`

Schema-wide display-unit projection over canonical marker storage. `unit-profiles.js` resolves every built-in marker for `EU` (International/SI), `ANZ` (Australia/New Zealand), and `US`; markers without a numeric difference use an identity projection. `secondary-unit-conversions.js` supplies compatible input and alternate clinical units. Import, manual entry, active data, ranges, reports, and AI context must use these central contracts rather than renderer-specific conversions.

**Key exports:** `UNIT_PROFILE_IDS`, `UNIT_PROFILE_LABELS`, `ANZ_UNIT_OVERRIDES`, `normalizeUnitProfile()`, `getUnitProfileLabel()`, `resolveMarkerUnitProfile()`, `convertCanonicalToDisplay()`, `convertDisplayToCanonical()`, `getMarkerInputUnits()`, `convertUnitInputToCanonical()`, `convertCanonicalToInputUnit()`, and `auditUnitProfileCoverage()`.

---

### `custom-marker-identity.js`

Creates and migrates category-independent `custom:*` IDs while preserving existing custom-marker dot keys. Legacy IDs are deterministic and migration is idempotent so offline devices converge.

**Key exports:** `createCustomMarkerId()`, `deriveLegacyCustomMarkerId()`, `ensureCustomMarkerIdentity()`, `migrateCustomMarkerIdentities()`, `getCustomMarkerId()`, `getCustomMarkerDotKey()`, and `resolveCustomMarkerDotKey()`.

---

### `marker-placement.js`

Pure marker placement engine and storage/display boundary. It validates visual category assignments, keeps unknown metadata forward-compatible, applies placement as the final active-data projection, and resolves mutation keys back to immutable storage locations.

**Key exports:** `migrateMarkerPlacements()`, `resolveMarkerIdentity()`, `getMarkerPlacementPlan()`, `setMarkerPlacement()`, `clearMarkerPlacement()`, `applyMarkerPlacements()`, `getMarkerStorageDotKey()`, and active-marker lookup helpers.

---

### `marker-terminology.js`

Generated, optional mapping registry for LOINC, NPU, NČLP, and UCUM metadata. It is not part of application startup, profile persistence, import/export, backup, sharing, or sync.

**Key exports:** `TERMINOLOGY_CATALOGS`, `MARKER_TERMINOLOGY_REGISTRY`, `getMarkerTerminologyMappings()`, and `findMarkerTerminologyMapping()`.

See [Laboratory terminology registry](/developers/marker-terminology) for the mapping and verification contract.

---

### `adapters.js`

Startup-safe parser adapter registry for specialty lab detection and normalization. It owns the shared specialty definitions; importer-only product catalogs such as Mosaic OAT/MOAT stay outside the startup graph.

**Key exports:**
- `ADAPTER_MARKERS` — flat object keyed by `"category.markerKey"`: `{ name, unit, refMin, refMax, categoryLabel, icon, group, singlePoint? }` (225 entries). Re-exported from `schema.js` as `SPECIALTY_MARKER_DEFS`
...
**Adapter registry:** array of `{ id, testTypes[], markers, productScoped?, detect?, normalize? }`:
- `fattyAcids` — 34 markers, detects Spadia/ZinZino/OmegaQuant by filename/text, normalizes to product-prefixed categories under the **Fatty Acids** sidebar group
- `metabolomix` — no unique startup markers; detects Genova Metabolomix+ and delegates importer-only normalization to stable `metabolomix*` categories
- `mosaicOat` — no startup markers; detects Mosaic/Great Plains OAT and MOAT reports, then uses the lazy 77-definition OAT or 21-definition MOAT catalog
- `oat` — 165 legacy/shared definitions; new imports are normalized to a laboratory-scoped marker space while existing legacy profiles remain readable
- `gutStool` — 3 shared stool/gut-barrier markers
...
`mosaic-oat-catalog.js` defines the importer-only Mosaic analyte order, aliases, sections, units, and OAT/MOAT marker metadata. `pdf-import-organic-acid-normalization.js` maps AI output into Mosaic, Metabolomix+, or lab-scoped OAT keys. Keep product identity in this deterministic post-AI layer; do not rely on model-generated category names to separate histories.
...
Chat-only source of truth for the base analyst instruction. Keeping it in the lazy Chat graph prevents prompt text from increasing the initial application bundle.

**Key export:** `CHAT_SYSTEM_PROMPT` — injected into normal and multi-persona chat requests before dynamic context and persona instructions.
...
- `state` — the mutable singleton:
  ```js
  {
    ...
    importedData: {            // all user data for the active profile
      ...
      customMarkers: {},       // { "cat.key": { markerId, name, unit, refMin, refMax, categoryLabel } }
      markerPlacements: {}     // { "gb:marker:*" | "custom:*": { categoryKey } }
    },
    unitSystem: 'EU',          // 'EU' | 'ANZ' | 'US'
...
- `callOpenAICompatibleLocalAPI(opts)` — compatibility re-export for Local AI request planning and adapter inference in `api-local.js`
...
- `hasAIProvider()` — returns `true` if any provider is configured; gates AI features
...
- `createProxyFetch(shouldUseProxy)` — compatibility fetch selector used by the shared OpenAI-format helper. Current Custom API calls pass `false`, so Custom validation, model discovery, and chat are browser-direct and do not fall back to a relay.

### `app-extension-runtime.js` and `app-extension-bootstrap.js`

Neutral single-extension boundary for trusted edition builds. The public/self-host bootstrap installs no extension, leaving the independently runnable core behavior unchanged. This is a build-time adapter contract, not dynamic plugin loading.

**Hook groups:** Settings slots/actions/lifecycle, onboarding slots/actions, AI ownership/authorization/transport/model policy/options/error mapping, voice request authorization/playback policy, sync keys/encrypted prefixes/conflict policy/post-apply callback, and startup.

An extension requires a stable ID. Active extension-owned AI transport fails closed when `authorizeRequest` is absent or does not return `true`.

### `unique-id.js`

Creates collision-resistant local record IDs with `crypto.randomUUID()` or `crypto.getRandomValues()`. `createUniqueId(prefix?)` throws when Web Crypto is unavailable instead of persisting a weak fallback identifier.
- `readWithStallTimeout(reader, label, timeoutMs?)` — guards one streaming read; Local AI uses the extended allowance only before its first token
- `STREAM_STALL_TIMEOUT_MS`, `LOCAL_AI_FIRST_TOKEN_STALL_MS`, `FETCH_REQUEST_TIMEOUT_MS`, `AI_IMPORT_REQUEST_TIMEOUT_MS` — transport timeout constants

**Window exports:** none

---

### Local AI provider modules

Local AI uses a normalized adapter layer so compatible endpoints work without hiding richer Ollama, LM Studio, and Unsloth Studio capabilities.

**Key modules and exports:**

- `local-ai-provider-registry.js` — `LOCAL_AI_PROVIDER_ADAPTERS`, `getLocalAiProviderAdapter()`, capability lookup, and detection precedence
- `local-ai-provider-ollama.js` — native discovery, streaming inference, context override, performance diagnostics, and unload
- `local-ai-provider-lmstudio.js` — native discovery/model enrichment, guarded unload/load-at-context, streaming compatible generation, native fallback, and truncation diagnostics
- `local-ai-provider-openai-compatible.js` — generic `/v1/models` discovery and `/v1/chat/completions` inference
- `local-ai-provider-shared.js` — base URL normalization, execution-location classification, compatible-model normalization, and secret redaction
- `local-ai-discovery.js` — short-lived discovery cache and loaded-model metadata updates
- `api-local.js` — prompt-token estimation, context/output replanning after a native load, adapter selection, and normalized inference diagnostics
- `local-ai-lifecycle.js` — memory-only runtime handoff and best-effort model release across same-machine endpoints

See [AI provider internals](/developers/ai-provider-internals#local-ai-adapter-contract) for the capability and lifecycle contract.

---

### Import model-testing modules

`import-benchmarks.js` captures durable, device-local diagnostics around production PDF/image imports. `import-reference-benchmark.js` runs and scores the bundled reference report. `settings-import-benchmark-controller.js` and the benchmark helpers in `settings-data.js` own the user-facing history and comparison UI.

See [AI model testing internals](/developers/ai-model-testing-internals) for record statuses, scoring layers, same-input comparison, persistence, and fixture maintenance.
...
**Key exports:**
- `getActiveData()` — builds or reuses the active projection: schema and custom markers, personal ranges, date-aligned values and entry context, phase/context ranges, deterministic calculated markers, unit conversion, and final display placements
- `invalidateActiveDataCache()` — clears the projection cache after a tracked mutation
- `saveImportedData()` — persists `state.importedData` to localStorage (or encrypted store), triggers backup
- `filterDatesByRange(data, options?)` — returns a date-filtered projection without mutating the cached active dataset
- `registerRefreshCallback(fn)` — registers the refresh function from `app-event-listeners.js`
- `getFocusCardFingerprint()` — hash of lab/profile inputs relevant to the Focus card
...
### Lab marker projection helpers

- `data-calculated-markers.js` — direct-result-first calculated ratios, indices, free water deficit, PhenoAge, Bortz Age, combined Biological Age, and dated calculation guidance.
- `marker-context-ranges.js` — whole-age-at-draw, sex/age band selection, collection-time parsing, and cortisol range resolution.
- `cycle-draw-phases.js` — normalization of explicit recorded phase, phase detail, cycle day, and recorded/predicted source.
- `marker-range-suggestions.js` — privacy-safe public issue payload and gating to untouched built-in ranges.
- `profile-marker-alias-migrations.js` — canonical dotKey migrations across values, snapshots, custom markers, ranges, notes, labels, and date-scoped manual metadata.

See [Lab markers and range internals](/developers/lab-markers-ranges-internals) for precedence and invariants.
...
- `getEffectiveRangeForDate(marker, dateIndex)` — dated phase, contextual optimal/reference, and static/personal range lookup
- `getEffectiveRangeLabelForDate(marker, dateIndex)` — matching user-facing source/policy label such as **Lab reference**, **Target**, or a phase label
- `getPhaseRefEnvelope(marker)` — widest span across all cycle phases for chart ref bands
- `getContextRefEnvelope(marker)` / `getContextOptimalEnvelope(marker)` — chart envelopes across dated context ranges
...
- Five Chart.js plugins (registered globally):
  ...
  - `phaseBandPlugin` — compact phase pills at observed points, with optional cycle day and recorded/predicted tooltip context
...
### Supplement and medication modules

The feature is split by responsibility. `supplements.js` orchestrates the editor and persistence; `supplement-medication-domain.js` owns identity, lifecycle, periods, schedules, quantities, units, and additive migration; `supplement-form-ui.js` and `supplement-import-controller.js` own the structured form and reviewed import workflow; `supplement-dashboard.js` and `supplement-impact.js` own current display and historical analysis; `supplement-quality.js`, `supplement-context.js`, and `supplement-warnings.js` own evidence and bounded projections.
...
- `pauseSupplement(idx)` / `endSupplement(idx)` — close the current period while preserving history
- `restartSupplement(idx)` / `beginSupplementDoseChange(idx)` — append or edit period-based lifecycle state
- `deleteSupplement(idx)` — removes by index
- `renderSupplementsSection()` — renders current records and claim-grouped mitochondrial evidence
...
- `getSupplementStatus()` / `getSupplementsOverlappingRange()` — period-derived current/history selection
- `buildSupplementAIContext()` — compact/detail source-quality context with hard character limits
- `parseSupplementQuantity(str)` — structured quantity parser; `parseAmount()` remains the impact compatibility wrapper
...
**Record model:** schema-v2 rows have stable IDs and explicit use periods. Product-label directions, active ingredients, other label materials, personal regimen, and source laboratory evidence remain separate. Legacy fields and unknown keys are retained for compatibility.
...
### Meals and Nutrition modules

The feature is lazy-loaded through `nutrition-context.js`. `nutrition-analysis.js` and `nutrition-ai-settings.js` own image-capable provider routing and editable extraction; `nutrition-store.js` owns the device-encrypted IndexedDB cache and timestamp-aware profile mirror; `nutrition-summary.js` owns coverage-aware windows and history; the benchmark modules own independent photos and per-model cancel/retry behavior.

**Key exports:**
- `openNutritionModule()` / `openNutritionHistoryModule()` — lazy Body/dashboard routes
- `analyzeMealPhoto()` / `buildMealAnalysisPrompt()` — meal-photo and nutrition-label structured extraction
- `getNutritionAIRoute()` / `listNutritionVisionModels()` — dedicated route and image-capable provider catalog
- `saveActiveProfileMeal()` / `deleteActiveProfileMeal()` — aligned canonical profile, cache, and tombstone operations
- `sanitizeNutritionMeal()` — thumbnail allowlist and transient/full-image stripping boundary
- `computeNutritionSummary()` / `computeNutritionHistory()` — 7/30/90-day summaries and 30D/3M/6M/1Y/All history
- `buildNutritionSummaryContext()` / `buildNutritionHistoryAnalysisPrompt()` — bounded aggregate context and editable history handoff
- `resolveNutritionTargets()` — fixed or weight-aware target projection
- `runModelComparison()` / `cancelComparisonRun()` — parallel benchmark lifecycle with individual cancellation

See [Meals and Nutrition internals](/developers/nutrition-internals) for the data record, full-size-image boundary, storage/sync reconciliation, request lifecycle, and required tests.
...
Health-context card rendering and lifecycle facade. It owns the grid, save-and-refresh flow, context notes, demo live-AI confirmation controls, card-tip launchers, and compatibility exports for the split editor, summary, dashboard-AI, and health-dot modules.

**Key exports:**
- `renderProfileContextCards()` — renders the responsive nine-card grid, progress, demo AI status, and additional-context notes
...
- `loadContextHealthDots()` — compatibility facade for stale-card assessment in `context-card-health-dots.js`
- `getCardFingerprint(key)` — djb2 hash of lab data, card data, shared notes/lens, sex, and DOB for cache invalidation
- Per-card editor functions: `openDiagnosesEditor`, `openDietEditor`, `openExerciseEditor`, `openSleepRestEditor`, `openLightCircadianEditor`, `openStressEditor`, `openLoveLifeEditor`, `openEnvironmentEditor`, `openHealthGoalsEditor`; Medical History helpers come from `context-card-medical-history-editor.js`, and lifestyle editors come through the cold-safe `context-card-lifestyle-editors.js` facade
- Matching save/clear functions for all nine context areas are re-exported from their owning editor modules
...
**Window exports:** none; context-card actions are module-only and use delegated data attributes or explicit lazy facades.

---

### `context-card-health-dots.js`

AI-backed profile summaries, colored health dots, and short marker-linked tips for context cards. It fingerprints and caches each card independently, sends only stale cards, normalizes model output, and falls back to gray on malformed or failed responses. Demo profiles use bundled precomputed insights; local models can refresh them automatically, while cloud or paid models require in-memory consent bound to the current demo profile, provider, and model.

**Key exports:** `getDemoContextAIMode()`, `enableDemoContextLiveAI()`, `disableDemoContextLiveAI()`, `applyDotColor()`, `applyAISummary()`, `applyAIProfileSummary()`, `getCardFingerprint()`, `loadContextHealthDots()`, `refreshAllHealthDots()`.

**Window exports:** none directly; compatibility functions are exposed through `context-cards.js`.
...
Cold-safe facade for all lifestyle context editors. It keeps the small diet-contaminant badge path available during dashboard render, lazy-loads `context-card-lifestyle-editors-impl.js` on first editor action, retries a failed module fetch once with a fixed alternate URL, and forwards the save/change callbacks owned by `context-cards.js`.
...
Main editor implementation for Diet & Digestion, Sleep & Rest, Exercise, Stress, Love Life & Relationships, and Environment. It renders the current option sets, progressive detail sections, SI/US hydration labels, diet contaminant badge, and save/clear handlers. Light, goals, interpretive lens, and the contaminant detail modal are delegated to `context-card-lifestyle-special-editors.js` and re-exported.

**Window exports:** none; called through the cold-safe lifestyle facade.

---

### `context-card-lifestyle-special-editors.js`

Specialized editors for Light & Circadian, prioritized Health Goals, Interpretive Lens, and diet contaminant details. It mirrors relevant Light setup values and keeps Health Goals sorted by priority.

**Window exports:** none; re-exported through the lifestyle editor modules and `context-cards.js`.
...
The Genome feature is a lazy-loaded 10-module family. `dna.js` is the public orchestration barrel; parsing, evidence, UI, mtDNA, and shell integration remain separated by responsibility.

- `dna-actions.js` — delegated Genome action handlers.
- `dna-evidence.js` — association/evidence/relevance labels, ranking, focused Ask AI prompts, publication labels, and privacy-safe catalog issue links.
- `dna-file-detection.js` — provider, annotated-report, 23andMe MT/Y, and Living DNA motherline classification.
- `dna-genotype.js` — allele/repeat normalization and strand-aware catalog matching.
- `dna-mtdna.js` — mutation parsing, parent/subclade resolution, mitochondrial climate-adaptation context, preview, persistence, and manual haplogroup entry.
- `dna-parser.js` — Web Worker raw-file parser plus deterministic annotated/clinical report extraction.
- `dna-runtime-bridge.js` — cold-safe function registry used before the Genome bundle is loaded.
- `dna-runtime.js` — lazy stylesheet, pending-import, shell refresh, profile, and chat dependencies.
- `dna-ui.js` — Genome section, raw/report/manual entry, association groups, evidence controls, and import previews.
- `dna.js` — catalog loading, persistence, APOE resolution, re-import merge rules, AI context assembly, and public bridge registration.

**Key exports:**

- `detectDNAFile(text)` — detects AncestryDNA, 23andMe autosomal/MT/Y, MyHeritage/FTDNA CSV, Living DNA autosomal/motherline, Illumina GSGT, and annotated SNP report formats by content.
- `parseDNAFile(file)` — scans a raw file in a Web Worker against the 60-entry `data/snp-health.json` catalog and returns normalized matches with association, evidence, and relevance metadata; preview/save resolve APOE from the two component calls.
- `parseClinicalSnpReportText()` / `parseManualSnpRows()` — deterministic report and small-list entry paths, including allele calls and supported numeric repeats such as `6/7`.
- `upsertGeneticsSnp(profileData, rsid, genotype, source)` — validates one catalog call and preserves source metadata for an explicit manual/report override.
- `saveGeneticsData(profileData, result)` — refreshes raw-file calls while preserving mtDNA and manual/report overrides; an overlapping explicit override remains authoritative.
- `buildGeneticsContext(genetics, activeMarkerKeys, options)` — serializes separately gated APOE/mtDNA, bounded priority findings, and optional all-call inventory. Full evidence scope/context is opt-in.
- `buildFullGeneticsContext(genetics)` — explicit genetics-question path with inventory and evidence details.
- `buildSnpAIInterpretationPrompt(rsid, stored, entry)` — focused, editable Ask AI baseline for one finding.
- `getRelevantSNPs(markerKey)` — marker-linked findings sorted by association, relevance, and evidence presentation rank.
- `renderGeneticsSection()` — Genome content with evidence/source/correction controls and raw/report/manual entry actions.
- `handleDNAFile(file)` / `handleSnpReportFile(file)` — preview and confirm flows for raw exports and report files.
- `parseMtDNAMutations()` / `resolveHaplogroup()` / `detectMtDNAMismatch()` — maternal marker parsing, specific-subclade resolution, and mitochondrial climate-adaptation latitude context.

The lazy public surface is registered through `configureDnaModuleBridge()`. Shell and dashboard callers use `health-data-loader.js` or `dna-runtime-bridge.js`; Genome functions are no longer installed as a broad set of `window` globals.
...
- `parseLabPDFWithAI(pdfText, fileName, onProgress)` — sends text + `buildMarkerReference()` to AI, extracts explicit collection time and fasting status, reports reading/writing phases, rejects truncated output, and maps complete results to marker keys
...
- `updateImportProgressPct(pct, stageLabel?)` — streaming AI progress and reading/writing phase-label hook
...
### `pdf-import-ai-utils.js`

Shared AI-import transport helpers.

**Key exports:**
- `callImportAIWithStreamFallback(request, label)` — retries browser-aborted import streams once without streaming
- `createImportAIProgress(options)` — estimates prefill from prior per-model performance, then switches from **Reading report** to **Writing results** on the first generated text
- `saveImportAIPerf(key, result)` — persists bounded per-model timing used by later progress estimates
- `tryParseJSON(text)` / `formatImportError(error)` — tolerant parse and user-facing error helpers; caller-side truncation checks run before review
- `IMPORT_COLLECTION_CONTEXT_PROMPT` / `normalizeImportedCollectionContext()` — accept only explicit specimen collection time and fasting state; reject processing/report-time substitution and clock-based fasting inference
...
### PDF import marker and unit helpers

- `pdf-import-marker-mapping.js` — canonical schema/custom/specialty matching, alias resolution, allowed calculated destinations, and the `buildMarkerReference()` prompt catalog.
- `pdf-import-ratio-units.js` — importable calculated-marker allowlist and SI-versus-conventional convention metadata for unitless report ratios.
- `pdf-import-unit-conversions.js` — valid-unit lists plus round-trip display/SI conversion for recognized and custom markers.
- `pdf-import-review-formatting.js` — responsive unit controls, viewport-safe picker positioning, and range formatting used by the review modal.

These helpers keep the modal presentation separate from normalization and commit semantics. A unit change must convert the value and range together; it must not relabel unchanged numeric data.
...
- `applyManualImportCollectionContext(patch)` — updates review-time `sampleTime` or tri-state `fasting`
...
### `pdf-import-commit.js`

Snapshot-aware import commit, re-review, deletion, range adoption, and collection-context ownership.

**Key contracts:**
- direct lab-reported ratios and indices normalize to their canonical **Calculated Ratios** destination; computed fallbacks fill only missing dates;
- active adopted ranges are recomputed by collection date and then import time, while manual overrides preserve a stashed lab fallback;
- `collectionContextSources` tracks `sampleTime` and `fasting` independently so same-date snapshot removal restores the newest remaining owner;
- mutation paths maintain marker/snapshot tombstones and roll back on durable-save failure.

**Window exports:** via `pdf-import.js` and import-history controls.
...
**Key exports:**
- `exportDataJSON()` — starts a v2 JSON export for the current profile, including entries, reviewed nutrition archive, targets, context, custom marker identities, `markerPlacements`, notes, supplements, biometrics, genetics, Light/Sun data, and optional supported chat state
- `buildClientExportObject(profileId, includeChat?, includeNutrition?)` — module-only helper that returns the reusable v2 single-profile export object used by JSON export and encrypted profile sharing. Normal exports include a sanitized thumbnail-only nutrition archive; profile sharing deliberately disables it. OAuth/wearable connection credentials, raw per-device wearable rows, and full-size meal images remain excluded.
...
- `collectReportData(options?)` — returns the versioned, detached report snapshot selected by report-builder options
- `exportPDFReport(options?)` — renders the selected report snapshot as printable HTML/PDF output
- `clearAllData()` — confirms and wipes all imported data for the current profile

**Window exports:** none; shell actions and runtime dependencies use module imports.

---

### `export-report-data.js`, `export-report.js`, and `export-report-html.js`

Report data is resolved once into a versioned, renderer-independent snapshot. `export-report-data.js` owns the pure schema, per-date range/status/provenance records, section minimization, and bounded agent projection. `export-report.js` owns active-state collection and the optional practitioner overview. `export-report-html.js` consumes the prepared snapshot for printable output and must not re-resolve report facts independently.

**Key exports:** `REPORT_DATA_SCHEMA_VERSION`, `buildReportDataSnapshot()`, `formatReportDataForAgent()`, `buildPreparedReportPayload()`, `collectReportData()`, `buildReportAgentContext()`, `generateReportAISummary()`, `exportPDFReport()`, and `buildReportHTML()`.

See [Report data contract](/developers/report-data).
...
On official hosts, new `vps1_...` IDs route directly to `https://shares.getbased.health/api/share`; older IDs and independent self-hosts retain the same-origin route. Passwords require at least 12 characters, the generated value contains 24 random bytes, and expiry is capped at 30 days.
...
Chat public barrel and startup entry point. Importing this module installs `chat-window-bindings.js` and re-exports the public helpers from the feature modules. The composer/draft, bounded transcript, scroll, edit/fork, recommendation-disclosure, persona persistence, and discussion-round modules keep durable state and high-churn UI behavior out of the public barrel. See [Chat internals](/developers/chat-internals) for the end-to-end ownership, storage, Sync, and backup contract.
...
**Window exports:** `sendChatMessage`, `setChatPersonality`, `openChatPanel`, `closeChatPanel`, `createNewThread`, `loadThread`, `deleteThread`, `renameThread`, `generateCustomPersonality`, `saveCustomPersonality`, `askAIAboutMarker`, `addImageAttachment`. The original-resolution attachment flow replaces the old HD toggle.

The v1.19.0 workspace adds `chat-layout.js` for resizing, `chat-model-controls.js` and `chat-model-preferences.js` for the provider-scoped model/effort picker, `chat-thread-list-view.js` for project/list markup, and `chat-message-avatars.js` for sender identity. Project persistence stays in the thread index. [CLI agent providers](/developers/cli-agent-providers#architecture-and-ownership) lists the separate Companion and agent-tool module owners.
...
Direct chat send and streaming state. Owns `sendChatMessage()`, Enter-key handling, thread-bound abort state, send-button icon mode, typewriter trickle, image attachment payload injection, API message assembly, usage footnotes, live EMF/product recommendation injection, and partial-message persistence on abort. It delegates textarea/draft behavior to `chat-composer.js`, concise assistive status to `chat-stream-status.js`, continuation handling to `chat-continuation.js`, and collapsed recommendation state to `chat-recommendation-disclosure.js`.
...
Chat transcript rendering. Owns `renderChatMessages()` for persisted messages, latest-turn and fork action placement, lens-source disclosures, assistant footnotes, image badges, EMF hints, and persisted recommendation sections. It renders only the range supplied by `chat-render-range.js`, delegates scroll ownership to `chat-scroll.js`, and reveals earlier batches without truncating stored history.

**Key exports:** `renderChatMessages`, `showEarlierChatMessages`, `revealChatMessage`, `_getNoDataPrompts`, `_renderLensSources`
...
Growing textarea and active-thread draft coordinator. It binds visible input to `{ profileId, threadId }`, restores drafts after async thread switches, clears a sent draft, and prevents a late decrypt from writing into a newly selected thread.

**Key exports:** `configureChatComposer`, `initChatComposer`, `resizeChatInput`, `saveChatDraft`, `getChatDraft`, `restoreChatDraft`, `clearChatDraft`, `resetChatComposer`, `setChatInputValue`.

**Window exports:** none.

---

### `chat-draft-storage.js`

Device-local draft cache and 300 ms debounced persistence. Draft keys use the sensitive storage wrapper but are intentionally excluded from Sync and every backup/export path. Its ordered write chain prevents a pending save from resurrecting a cleared draft.

**Key exports:** `chatDraftStorageKey`, `getCachedChatDraft`, `loadChatDraft`, `rememberChatDraft`, `clearStoredChatDraft`, `flushStoredChatDraft`.

**Window exports:** none.

---

### `chat-render-range.js`, `chat-scroll.js`, and `chat-stream-status.js`

Transcript-scale and interaction helpers. `chat-render-range.js` exposes the latest 120 records and expands in 120-record batches; `chat-scroll.js` follows streaming only while the user remains near the latest message; `chat-stream-status.js` emits concise ARIA status changes without making every streamed token a live-region update.
...
Discussion round execution loop. Owns sequential per-persona API calls, progress and assistive status, pause/abort state, origin-thread persistence, typewriter updates, usage footnotes, partial-response preservation, and targeted failure records. It returns the remaining participants so resume and per-participant retry do not replay successful responses.
...
Multi-persona discussion picker controls. Owns persona picker markup, active-persona locking, one-at-a-time participant addition, request-count/charge disclosure, keyboard dismissal, checkbox limiting, and picker selection reads.
...
Personality selection and workflow facade. It renders the picker and chat header, invokes the dedicated custom-persona dialog, streams an editable generated draft, persists through `chat-personality-storage.js`, and updates the active thread's personality metadata.

**Key exports:** `getActivePersonality`, `getCustomPersonalities`, `loadCustomPersonalities`, `saveCustomPersonalities`, `getCustomPersonality`, `setChatPersonality`, `loadChatPersonality`, `updateChatHeaderTitle`, `updateChatHeaderModel`, `updatePersonalityBar`, `togglePersonalityBar`, `generateCustomPersonality`, `saveCustomPersonality`, `startNewCustomPersonality`, `editCustomPersonality`, `deleteCustomPersonality`.

**Window exports:** assigned by `chat-window-bindings.js` for existing inline handlers and cross-module callbacks.

---

### `chat-personality-editor.js`

Custom persona modal and host policy. The official `getbased.health` host requires a versioned acknowledgement that a persona is an AI-generated interpretation and cannot be used to impersonate or imply endorsement. Self-hosted origins receive the responsibility notice without a required checkbox.

**Key exports:** `isOfficialHostedPersonaApp`, `hasCurrentPersonaAgreement`, `isCustomPersonalityUsable`, `buildPersonaAgreementRecord`, `openPersonaEditorDialog`.

**Window exports:** none.

---

### `chat-personality-storage.js` and `chat-personality-merge.js`

Encrypted-wrapper persistence, synchronous decrypted cache, deletion tombstones, and deterministic per-persona Sync merge. Persona updates resolve by `updatedAt`/`createdAt`; a tombstone at least as new as the record removes it without replacing unrelated personas.

**Key exports:** storage-key helpers, load/save/cache helpers, `recordCustomPersonalityDeletion`, `customPersonalityUpdatedAtMs`, and `mergeCustomPersonalityState`.
...
Conversation index CRUD, active-thread switching, linked fork creation, rail rendering, and thread deletion tombstones. `pruneOldThreads()` is a compatibility no-op. An existing index that cannot be decrypted or parsed blocks writes and new-chat creation rather than replacing saved conversations with an empty index.

**Key exports:** `loadChatThreads`, `saveChatThreadIndex`, `ensureActiveThread`, `createNewThread`, `createForkedThread`, `switchToThread`, `deleteThread`, `renameThread`, `renderThreadList`, and the search compatibility exports.

**Window exports:** assigned through `chat-window-bindings.js` where legacy callers still require them.
...
Chat message action rendering and delegated handlers. It owns Retry/Regenerate, Copy, Listen, context details, responsive overflow actions, discussion pause/resume/retry controls, and callbacks into latest-message editing and linked forks. Voice actions cross the lazy boundary through `voice-loader.js`.

**Key exports:** `configureChatMessageActionDeps`, `installChatMessageActionDelegates`, `buildActionBar`, `buildUserActionBar`, `buildForkSourceNotice`, `regenerateLastMessage`, `copyMessage`, `toggleContextDetails`.

**Window exports:** assigned by the module directly and re-exported by `chat.js` for existing inline handlers.

---

### `chat-message-edit.js`

Latest-turn inline editor and non-destructive assistant-reply forks. Edit-and-retry is limited to the latest visible text-only user message while no response is streaming; fork creation copies normalized history through the selected assistant message into a linked thread.

**Key exports:** `configureChatMessageEditDeps`, `getLatestUserMessageIndex`, `beginChatMessageEdit`, `cancelChatMessageEdit`, `submitChatMessageEdit`, `prepareChatMessageEditSend`, `forkChatFromMessage`.

**Window exports:** none; `chat-actions.js` receives callbacks.

---

### `chat-recommendation-disclosure.js` and `chat-storage-safety.js`

`chat-recommendation-disclosure.js` persists related suggestions collapsed and marks only the first or materially changed slot set as new. `chat-storage-safety.js` normalizes untrusted chat, thread, persona, tombstone, discussion/fork, usage, and image shapes before import, restore, or Sync apply.
...
Private-transport attestation display helpers shared by the chat header and assistant-message cost footnotes. Tinfoil uses **🔒 verified** after its full policy passes. Venice uses **🔒 TEE + GPU** after its required DCAP and signed NRAS checks pass, while the tooltip keeps code-measurement, co-location, and response-source limits visible.
...
Voice is a first-use chat subsystem rather than part of the startup bundle. `voice-loader.js` keeps microphone capture and speech dependencies cold until the user records, listens, or enables automatic reading. It binds each pending action to the active chat thread and message so late transcription or speech work cannot update a stale conversation.

**Chat orchestration:**

- `voice-controller.js` owns microphone state, transcript insertion, per-message playback, automatic reading, cancellation, and thread/message identity checks.
- `voice-capture.js`, `voice-player.js`, `voice-audio.js`, and `voice-text.js` own ephemeral recording, progressive audio playback, PCM/WAV handling, and safe speech-text normalization/chunking.
- `voice-service.js` and `voice-provider-registry.js` select the configured STT/TTS provider without coupling chat controls to provider implementations.

**Provider paths:**

- `voice-provider-browser-local.js` runs downloaded Whisper STT and Kokoro TTS through `voice-local-stt-worker.js`, `voice-local-tts-worker.js`, and the serialized `voice-local-engine.js` worker controller.
- `voice-provider-local-server.js` calls OpenAI-compatible `/v1/audio/transcriptions`, `/v1/audio/speech`, and `/v1/models` endpoints directly from the browser.
- `voice-provider-ai-cloud.js` supplies the xAI adapter plus the OpenRouter, PPQ, and Venice audio paths; `voice-provider-elevenlabs.js` supplies ElevenLabs. Both use `voice-provider-cloud-shared.js` for browser-direct calls after cloud-processing consent.
- `voice-provider-catalog.js` owns capability and execution-location metadata; `voice-model-catalog.js` pins local models, voices, languages, download sizes, and CPU/WebGPU modes.

**Settings and storage:**

- `settings-voice-panel.js`, `settings-voice-view.js`, `settings-voice-model-controller.js`, and `settings-voice-hardware.js` own the **Settings → Voice** UI, downloads, connection tests, voice catalogs, and hardware guidance.
- `voice-settings-schema.js` is the authority for device-local, syncable, secret, and backup keys. `voice-settings-storage.js` validates preferences and delegates secret writes to encrypted provider storage.
- Downloaded model assets and fetched voice catalogs are rebuildable caches. They are not sync payloads or backup data.

**Network boundary:** cloud voice is direct browser transport. Do not document or restore the removed `/api/voice` relay without a new reviewed privacy and consent design.

**Key exports:** `loadVoiceModule`, `toggleVoiceRecording`, `toggleMessageSpeech`, `maybeAutoReadAssistantMessage`, `stopVoiceActivity`, `getVoiceSettings`, `setVoiceSetting`, `transcribeVoice`, `createVoiceSynthesizer`

**Window exports:** none; chat and shell actions use module callbacks.

---

### `settings.js`

Settings modal shell with profile, display, AI, Voice, privacy, data, wearables, and Agent Access tabs. Delegates AI provider panels to `provider-panels.js`, Voice rendering and actions to `settings-voice-panel.js`, wearables rows to `wearables-settings-panel.js`, sync controls to `settings-sync-panel.js`, and Agent Access rendering/actions to `settings-agent-access-panel.js`.
...
- `setUnitSystem(system)` — `'EU'` | `'ANZ'` | `'US'`
...
Cold facade for the Cross-device Sync and Agent Access settings graph. A first Settings render returns marked loading placeholders and starts `settings-sync-panel-impl.js`; when the import resolves, the facade replaces both placeholders in place so the real controls appear without closing and reopening Settings. Action calls use the same loader and retry boundary.
...
- `hydrateSettingsSyncPanel()` — forwards post-paint mnemonic and relay hydration after the implementation loads
- `showSyncSetupModal()` — loads the implementation and opens the setup wizard
...
### `settings-sync-panel-impl.js`

Loaded implementation for the Settings → Data sync section. It owns setup/restore UI, mnemonic and relay controls, diagnostics entry points, and Agent Access delegation. Existing-mnemonic joins call the controlled restore path in `sync-identity.js`.

**Key exports:** `configureSettingsSyncPanelDeps`, `renderSyncSection`, `renderMessengerSection`, `showSyncSetupModal`, `closeSyncSetup`, `closeRestoreMnemonicDialog`, `hydrateSettingsSyncPanel`.

**Window exports:** none directly; the facade owns compatibility bindings.
...
Shared, DOM-independent ordering for lab navigation and category controls. `getLabCategoryEntriesInSidebarOrder()` keeps regular categories first and preserves schema order within specialty groups so the placement picker and sidebar agree.

---

### `marker-detail-placement.js`

Marker-detail category picker and persistence controller. It lists only destinations accepted by `marker-placement.js`, groups categories present in the profile before other compatible categories, preserves sidebar order, serializes per-profile mutations, and provides restore-to-native behavior.

**Key exports:** `configureMarkerDetailPlacement()`, `getMarkerPlacementChoices()`, `renderMarkerPlacementSummary()`, `openMarkerPlacementModal()`, `saveMarkerPlacement()`, and `restoreMarkerPlacement()`.
...
Startup-loaded Health & Data feature side-effect imports extracted from `app-feature-modules.js`. This module preserves the charts/notes/supplements/recommendations/cycle/context/DNA/wearables order and installs the cold nutrition facade while keeping the full Meals & Nutrition feature lazy.
...
Passphrase encryption, browser-device credential wrapping, and cross-tab data-change notifications.
...
With passphrase protection off, supported credential keys use a `d1:` AES-GCM envelope backed by a non-extractable key in IndexedDB. If device wrapping is unavailable, credential persistence fails closed to memory-only behavior. BroadcastChannel never carries the encryption key; each tab unlocks independently.
...
IndexedDB auto-backup, folder backup via File System Access API, manual database backup, and restore. `backup-chat-storage.js` supplements profile collection with raw thread index/message envelopes so a locked or partially recovered index cannot make a full backup omit ciphertext. Unsent draft keys are intentionally excluded.

**Key exports:**
- `scheduleAutoBackup()` — debounced five-minute trigger; saves up to 5 snapshots to IndexedDB + writes to folder backup if configured
- `buildBackupSnapshot()` — legacy synchronous localStorage snapshot retained for focused tests and compatibility
- `buildFullBackupSnapshot()` — asynchronously captures IndexedDB-backed profile blobs, chat records, preferences, and device-local data
- `exportEncryptedBackup()` / `importEncryptedBackup()` — manual full-database bundle export and restore
- `getAutoBackupSnapshots()` — reads the retained IndexedDB snapshot list
- `restoreAutoBackup(id)` — restores localStorage plus profile, wearable, and cycle IndexedDB records, then reloads
- `initFolderBackup()`, `pickFolderForBackup()`, `reauthorizeFolderBackup()`, `removeFolderBackup()` — File System Access lifecycle for latest and daily folder snapshots
...
**Key exports:**
- `buildLabContext()` — serializes enabled lab groups, effective dated ranges, explicit collection context, profile/context cards, cycle data, notes, and other enabled sources into tagged AI context

**Window exports:** none; callers import it directly or use a configured module facade.

---

### `lab-context-collection.js`

Builds `[section:labCollectionContext]` from explicit per-draw metadata. It emits collection time, tri-state fasting when known, cycle day, and recorded phase; missing fields are omitted with an instruction not to infer them.

**Key export:** `buildLabCollectionContextSection(entries)`.

**Window exports:** none.

---

### `lab-context-change-timeline.js`

Builds the `[section:changeHistory]` profile-context timeline used by `buildLabContext()`. It filters the timeline by enabled Insight Context Cards and Light & Sun sources, orders snapshots by date, and renders field-level changes.

**Window exports:** none.

---

### `lab-context-wearables.js`

Builds compact wearable context and optional Agent Access series. It also detects contradictions between manually selected Sleep & Rest duration/quality and recent wearable data, returning an explicit mismatch warning instead of silently choosing one source.

**Key exports:** `getSleepContextMismatch()`, `buildWearableContext()`, `buildWearableSeriesSection()`, and the in-app/Agent Access wearable-context setting helpers.
...
**Key exports:**
- `ADAPTERS` — array of `{id, displayName, authType, oauth?, apiHost?, metrics, accountInfo?}` for the 9 registered sources (Oura / Withings / Ultrahuman / WHOOP / legacy Fitbit / Google Health / Polar / Apple Health / manual)
...
- `applyOAuthConfigured(map)` / `isOAuthAdapterConfigured(adapterOrId)` — apply and read server-confirmed capability flags; Google Health uses this gate independently from its client-ID override
...
- `loadWearableRuntimeConfig()` — POSTs `{wearable_runtime_config: true}` to `/api/proxy`, applies returned public client-ID overrides and capability booleans, and clears failed cached requests so a later scheduler pass can retry. A 1.5s soft timeout keeps startup non-blocking
...
Each connected source ships as a pair: `wearables-<vendor>.js` (read API) + `wearables-<vendor>-auth.js` (OAuth dance). Apple Health and Manual skip the auth half. Tokens are always encrypted in the per-profile device-local vault and are excluded from sync and backups. They do leave the browser transiently when the selected OAuth exchange, refresh, or provider call requires the operated or self-hosted compatibility endpoint.

**OAuth2 confidential clients:** Oura, Withings, Polar, Google Health, and WHOOP inject the deployment secret at the compatibility endpoint. Google Health and WHOOP additionally require explicit server capability gates; Google Health credentials and raw rows use the always-on device-local key.

**Other OAuth paths:** Ultrahuman follows its reviewed experimental self-host contract. Fitbit's PKCE modules remain only for already-connected legacy users; the adapter is hidden for new connections and has no reconnect flow.
...
- `supplement-warnings.js` — claim-level, primary-study mitochondrial evidence matching and bounded context; `food-contaminants.js` separately owns deterministic diet-source warnings.
...
- `sync-push.js` — outbound profile push path, missing-profile-data rejection, unchanged-payload suppression, generation-safe dirty clearing, exact source-commit tracking, relay-storage refresh scheduling, in-flight watchdog, and Evolu profileData insert/update.
...
- `sync-pull.js` — inbound pull orchestration and row loop; force-publishes pending backup restores, flushes dirty local work, tracks source-tab commits, and delegates one-time cleanup, active-profile UI refresh, and rebroadcast scheduling to helper modules.
...
- `sync-pull-active-refresh.js` — active-profile in-memory update, migration, chat reload, sidebar rebuild, current-view refresh, origin-aware toast, and sync-applied event after a pull.
...
- `sync-chat-apply.js` — inbound chat thread/message application, thread/persona delete tombstones, per-persona timestamp merge, and local freshness locks that protect just-edited local chat from stale remote rows.
...
- `sync-evolu8-candidate.js` — Evolu 8 default runtime facade; performs the verified one-time Evolu 7 identity handoff, retains an explicit V7 rollback, keeps identities aligned during restore/reset, and safely reclaims superseded V8 database generations.
- `sync-evolu8-identity-vault.js` — durable IndexedDB-backed owner ID and mnemonic handoff; valid commits let normal V8 startup avoid loading the V7 worker.
- `sync-identity.js` — BIP-39/QR lazy loaders plus mnemonic read/restore helpers; existing-identity joins suppress the client's internal reload, persist enabled owner state, then perform one controlled reload.
- `sync-settings-state.js` — persisted configured/active/paused state. `labcharts-sync-enabled` means an identity is configured; `labcharts-sync-paused` suppresses traffic without forgetting it.
- `sync-lifecycle.js` — runtime start, identity-preserving pause/resume, explicit disconnect/reset, and backup-restore push preflight.
- `sync-backup-restore-state.js` — durable restored-profile handoff; clears stale delta/cutover state, marks profiles dirty, and retains the retry list until a recovery push succeeds.
- `sync-subscriptions.js` — Evolu profile/settings subscriptions and debounce timers; paused state clears and blocks inbound delivery until resume rebinds them.
- `sync-dirty-state.js` — durable per-profile edit generations, captured-generation clearing, and dirty-state inspection for pull and compaction safety.
- `sync-origin-state.js` — tab-local committed `syncedAt` tracking used to distinguish source-browser echoes from updates received elsewhere.
- `sync-profile-fields.js` — deterministic shared profile metadata field list used by outbound payload collection and inbound profile merge.
...
- `sync-actions.js` — user-triggered sync actions, all-profile initial push, dirty-profile compaction preflight, fail-closed unreadable-profile guard, relay snapshot rebuild, and compatibility re-exports for save hooks and storage cleanup.
- `sync-save-hooks.js` — save/chat/profile-metadata debounce hooks, durable dirty marking, AI-setting push scheduling, per-profile timer cleanup, and canonical IndexedDB-backed profile reads that return `null` rather than an empty default on failure.
...
- `sync-payload-collectors.js` — Local AI/chat/display settings collection for sync payloads, including thread messages/tombstones, custom personas, persona tombstones, and synced setting allowlists.
- `sync-payload.js` — Evolu wire-payload helpers; deterministic profile-field assembly, Phase 2 cutover payload shape, gzip envelope handling, inbound payload parsing, and local-only data stripping for wearable credentials and SNP map rows.
