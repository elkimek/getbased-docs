...
Modules live under `js/`. This page is a guided implementation map, not a promise that every small hook/delegate/helper file has its own long section. It is grouped by layer — lower layers have no dependencies on higher ones.
...
Stable compatibility facade for marker schema consumers plus unit conversions, optimal ranges, pricing, and environment thresholds. Marker definitions are authored in category modules under `marker-schema/`, composed by `marker-schema/index.js`, and generated into `marker-schema.js`.
...
- `BUILTIN_MARKER_IDENTITIES`, `BUILTIN_MARKER_DOT_KEY_ALIASES`, `getBuiltinMarkerId()`, `getBuiltinMarkerDotKey()`, `resolveBuiltinMarkerDotKey()` — stable identity and legacy-location compatibility exports from the generated catalog
...
- `local-ai-provider-lmstudio.js` — native discovery/model enrichment, context-aware inference, performance diagnostics, and unload
...
- `api-local.js` — prompt-token estimation, context/output planning, adapter selection, and normalized inference diagnostics
...
- `sync-pull.js` — inbound pull orchestration and row loop; flushes dirty local work, tracks source-tab commits, and delegates one-time cleanup, active-profile UI refresh, and rebroadcast scheduling to helper modules.
