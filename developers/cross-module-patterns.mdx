---
title: "Cross-module patterns"
description: "Window exports, circular dependency avoidance, and shared state patterns in getbased."
---
...
**Never use `innerHTML` with concatenated user input without `escapeHTML`.** The shared `renderMarkdown()` function in `markdown.js` validates URLs against an allowlist (`http`, `https`, `mailto`) before rendering link tags; chat renderers call that shared boundary rather than implementing their own Markdown path.
