...
## Dependency and compiler upgrade gates

Runtime dependency bumps are code changes when they affect a security boundary. The proxy's exact Undici pins and their rationale are documented under [Transport dependency pins](/developers/deployment#transport-dependency-pins). For either the direct proxy transport or the nested `@vercel/blob` transport, run:

```bash
npm ci
npm test -- tests/proxy-network.test.js tests/proxy-upstream.test.js tests/proxy-entrypoint-runtime.test.js tests/proxy-rate-limit.test.js
npm run typecheck:server
npm run supply-chain:check
```

`npm ci` should finish with no known vulnerabilities. Do not make the exact-version assertions permissive just to accept a bot update: they force a deliberate review of the transport that production will load. GitHub Actions owns the exhaustive Chromium and combined-coverage run after the focused local checks pass.

The repository also intentionally remains on the TypeScript 6 compiler API. `scripts/architecture-map.mjs`, `scripts/app-shell-budget.mjs`, `scripts/dom-sink-audit.mjs`, and `scripts/strict-null-ratchet.mjs` import `typescript` programmatically. TypeScript 7's native release does not ship that API, so replacing the existing package with TypeScript 7 breaks these gates even when the `tsc` command itself works.

A future TypeScript 7 migration should be a dual-toolchain change: use the native package for the `tsc` CLI and keep `@typescript/typescript6` available for programmatic imports. Run every typecheck, the strict-null ratchet, architecture check, and quality gate before changing the compiler pin. Do not merge a generated single-package major bump as-is. See the [TypeScript 7.0 migration guidance](https://devblogs.microsoft.com/typescript/announcing-typescript-7-0/).
...
| `tests/proxy-entrypoint-runtime.test.js` | **Vitest** — production proxy entrypoint, Blob SDK artifact pin, preflight behavior, and limiter-boundary loading |
| `tests/proxy-network.test.js` | **Vitest** — public-DNS validation, exact Undici transport pin, socket lookup binding, dispatcher cleanup, and abort behavior |
| `tests/proxy-rate-limit.test.js` | **Vitest** — distributed Blob-backed and bounded local proxy rate-limit behavior |
| `tests/proxy-upstream.test.js` | **Vitest** — upstream proxy request routing through the protected network transport |
...
| `tests/test-venice-e2ee.js` | Venice encrypted mode: ECDH/AES-GCM, fail-closed responses, binding-level attestation UI, TEE headers, model detection |
