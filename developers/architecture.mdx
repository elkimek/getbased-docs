...
## Native-module philosophy

getbased has no app bundler: the browser loads native ES modules (`<script type="module">`) directly. The shipped JS remains human-readable as files, and most UI work is still edit → reload.
...
Vercel also runs `node scripts/fetch-catalog.mjs` before deployment. Treat the project as **zero-bundler**, not as a tooling-free repository.
