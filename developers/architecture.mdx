...
api/share.js        — bounded legacy-share compatibility and VPS redirect adapter
...
AI providers (OpenRouter, Routstr, PPQ, Venice, Local AI) are called directly from the browser for normal chat/import flows. Same-origin API helpers under `api/` cover hosted infrastructure such as OAuth/runtime proxying and bounded compatibility for legacy encrypted profile shares, but they must not receive AI prompts or plaintext profile data unless explicitly documented for a new feature. New encrypted share envelopes go directly to the isolated VPS service.
