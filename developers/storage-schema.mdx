...
| `labcharts-{profileId}-chat-threads` | JSON array (may be encrypted) | Thread index with timestamps/count, persona, discussion state, and optional fork-source metadata |
...
Unsent `chatDraft_*` records are intentionally absent from full database, auto, folder, and single-profile backups. Sent conversations and custom personas are included.
