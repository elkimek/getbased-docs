...
New official-app records use ids shaped as `vps1_{24-base64url-characters}` and are stored by the isolated profile-share service in a SQLite object table. Logical paths remain `profile-shares/v2/{id}.json`; separate expiry markers support bounded cleanup, and hourly abuse markers use a server-keyed daily HMAC instead of a reusable plain IP hash. The service limits envelopes to 3.75 MB, links to 30 days, and the database to 512 MiB while reserving host disk space. Local development stores the same logical share record in the `dev-server.js` in-memory map.

The operated SQLite database has no retained backup. This avoids restoring deleted or expired links, at the cost of outstanding temporary links becoming unavailable after storage loss. Do not copy the main database file while WAL mode is active if this policy changes; first add deletion tombstones and use SQLite's backup mechanism or another transactionally consistent snapshot with a tested restore and bounded retention.

The previous private Vercel Blob store remains read/delete-only during a configured transition of at most 31 days. `api/share.js` redirects new writes and missing legacy reads to the VPS, then stops reading Blob entirely at the fixed deadline. Opaque record bodies and ids are not migrated.

```js
{
  id: "vps1_base64url-share-id",
