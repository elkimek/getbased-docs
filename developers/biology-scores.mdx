...
Score definitions stay keyed by canonical storage paths even when a profile displays a marker in another category. `resolveActiveMarkerPath()` finds the projected active marker and `getMarkerStorageDotKey()` restores its immutable mutation/scoring path, so visual placement cannot change score inputs or create a second observation.
...
- specialty adapter counts: legacy/shared OAT and Metabolomix-style definitions 165, fatty-acid definitions 34, BioStarks markers 23, plus importer-only Mosaic catalogs with 77 OAT and 21 MOAT definitions;
