...
If any preflight push fails, compaction must not begin. If deletion succeeds but rebuilding is interrupted, the UI keeps a **Retry rebuild** path so the same canonical device can finish without deleting local profile data.
