...
Evolu 8 cannot open Evolu 7's local SQLite database directly. On the first V8 start without a committed identity vault, `sync-evolu8-candidate.js` therefore opens the V7 client without relay transports, reads its owner mnemonic, derives the V8 owner, and refuses the handoff if the owner IDs differ. It then commits the verified owner ID and mnemonic to `sync-evolu8-identity-vault.js` before starting the V8 database. Later normal starts read the vault and do not load the V7 worker.
...
- verify a first V8 start derives the same owner ID from the V7 mnemonic and commits the identity vault before V8 starts;
